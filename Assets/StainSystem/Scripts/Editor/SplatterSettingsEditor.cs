	...
	public class SplatterSettingsEditor : Editor {
		...
		public override void OnInspectorGUI() {
			...
			ShowHint("Splatters consist of multiple branches (long lines of particles). This value is the mean " +
					 "number of branches in each splatter.");
			...
			s.scaleMean = EditorGUILayout.Slider("Branch scale", s.scaleMean, 0f, 10f);
			...
			ShowHint("The way colors of branches are picked.\n" +
					 "Continuous: aka Rainbow mode. In this mode colors are calculated from the HSV space, where hue " +
			...
			switch (s.colorMode) {
				case ColorMode.Continuous:
					...
					break;
				case ColorMode.List:
					ShowHint("List of colors that will be used for branches. Each branch will pick a random color from this list.");
			...
			if (s.mode == SettingsMode.Mesh) {
				...
				s.multiplyColor = EditorGUILayout.Toggle("Multiply blending", s.multiplyColor);
			}
			...
			if (showColorsAdvanced) {
				...
				ShowHint("Maximum random value that will be added to brightness of branch color in HSV space.");
